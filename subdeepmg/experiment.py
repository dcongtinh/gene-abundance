...
from sklearn.decomposition import PCA, NMF, FastICA, FactorAnalysis, TruncatedSVD
from sklearn.preprocessing import MinMaxScaler, QuantileTransformer, PowerTransformer, RobustScaler, StandardScaler
...
from phucFSM import lowVarianceFilter, anovaFValue, ridgingLowVarianceFilter, madFilter, perceptronCheat
def para_cmd ():
	'''
	...
	parser.add_option('--log_file',help='For logging out selected features')
...
def reduce_dim (options,prefix_file_log, seed_value, index, train_x, val_x, v_data, train_y=None, val_y=None):
	'''
	...
	if options.algo_redu ==  "rd_pro": #if random projection
	...
	elif options.algo_redu == 'mds': # MultiDimensional Scaling (error: lack of attr 'MDS')
		...
		transformer = RFE(lreg, options.new_dim)
		train_x = transformer.fit_transform(train_x)
		...
		transformer = TruncatedSVD(n_components=options.new_dim, random_state=2020)
		train_x = transformer.fit_transform(train_x)
		print('Shape of data=' + str(train_x.shape))
		#val_x = transformer.transform(val_x)
	
	elif options.algo_redu == 'isomap':
		transformer = Isomap(n_components=options.new_dim, n_jobs=-1)
		train_x = transformer.fit_transform(train_x)
		#val_x = transformer.transform(val_x)
	
	elif options.algo_redu == 'lda': #LinearDiscriminantAnalysis
		transformer = LinearDiscriminantAnalysis(n_components=options.new_dim)
		train_x = transformer.fit_transform(train_x)
		#val_x = transformer.transform(val_x)
		print("shape of data=" + str(train_x.shape))
	
	# elif options.algo_redu == 'umap':
	#     transformer = umap.UMAP(n_neighbors=5, min_dist=0.3, n_components=options.new_dim)
	#     train_x = transformer.fit_transform(train_x)
	#     #val_x = transformer.transform(val_x)

	elif options.algo_redu == 'lasso': # Lasso Regression
		# scaler = StandardScaler()
		# train_x = scaler.fit_transform(train_x)
		transformer = SelectFromModel(estimator=LogisticRegression(C=1, penalty='l1', solver='saga'), threshold=-np.inf, max_features=options.new_dim).fit(train_x, train_y)
		train_x = transformer.transform(train_x)
		print("shape of transformed="+ str(train_x.shape))
		#val_x = transformer.transform(val_x)
	elif options.algo_redu == 'ridge': # Ridge Regression
		# scaler = StandardScaler()
		# train_x = scaler.fit_transform(train_x)
		transformer = SelectFromModel(estimator=LogisticRegression(C=1, penalty='l2'), threshold=-np.inf, max_features=options.new_dim).fit(train_x, train_y)
		train_x = transformer.transform(train_x)
		print("shape of transformed="+ str(train_x.shape))
		#val_x = transformer.transform(val_x)
	elif options.algo_redu == 'lvf':
		...
		train_x, val_x = anovaFValue(train_x, val_x, options.new_dim, train_y)
		blIsUsedPhucFSM = True
	elif options.algo_redu == 'rilv': # Ridge and Lowvariance filter mixed
		train_x, val_x = ridgingLowVarianceFilter(train_x, val_x, options.new_dim, train_y, val_y)
		blIsUsedPhucFSM = True
	elif options.algo_redu == 'mad':
		train_x, val_x = madFilter(train_x, val_x, 4096, train_y, val_y)
		blIsUsedPhucFSM = True
	else:
		print  (utils.textcolor_display (' this algo_redu ' + str(options.algo_redu) + ' is not supported!'))
		exit()   
	#tranform validation and test set
	if not blIsUsedPhucFSM: # use other methods
		if options.algo_redu in ['mds']:
			val_x = transformer.fit_transform(val_x)
		else:
			val_x = transformer.transform(val_x)
	if options.test_ext == 'y':  
		if options.algo_redu in ['mds']:
			v_data = transformer.fit_transform(v_data)
		else:
...
def run_kfold_deepmg(options,args):
	...
	for seed_value in range(options.seed_value_begin,options.seed_value_begin+options.run_time) :
		...
		for index, (train_indices,val_indices) in enumerate(skf.split(data, labels)):
			...
			#print  (train_x.shape)
			##### reduce dimension 
			unq_val, unq_cnt = np.unique(val_y,return_counts=True)
			print('Validate label info: ', unq_val, unq_cnt)
			if not (options.algo_redu in  ['','none']): #if use reducing dimension
				train_x, val_x, v_data = reduce_dim (options,prefix_file_log, seed_value, index, train_x, val_x, v_data, train_y=train_y, val_y=val_y)
...
def run_holdout_deepmg(options,args, special_usecase = '', txt_time_pre =''):
	...
	for seed_value in range(options.seed_value_begin,options.seed_value_begin+options.whole_run_time) :
		...
		for index in range(0,1):
			...
			if not (options.algo_redu in  ['','none']): #if use reducing dimension
				train_x, val_x, v_data = reduce_dim (options,prefix_file_log, seed_value, index, train_x, val_x, v_data, train_y=train_y)
				#apply cordi to new size
				#when appyling to reduce dimension, no more data which still = 0, so should not use type_dat='pr'
