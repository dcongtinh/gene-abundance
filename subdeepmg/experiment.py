...
from phucFSM import varianceFilter, perceptronCheat, anovaFValue
def para_cmd ():
    '''
...
def reduce_dim (options,prefix_file_log, seed_value, index, train_x, val_x, v_data, train_y=None, val_y=None):
    ...
    elif options.algo_redu == 'bfe': # Backward Feature Elimination #not working
    ...
    elif options.algo_redu == 'vf':
        # use low variance filter, vf for short
        train_x, val_x = varianceFilter(train_x, val_x, options.new_dim)
        blIsUsedPhucFSM = True

    elif options.algo_redu == 'pc':
        # use perceptron cheat
        train_x, val_x = perceptronCheat(train_x, val_x, options.new_dim, train_y)
        blIsUsedPhucFSM = True
    elif options.algo_redu == 'fval':
